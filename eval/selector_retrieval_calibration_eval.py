...
def write_markdown(report: dict[str, Any]) -> None:
    lines = [
        ...
        "## Case Summary",
        "",
        "| Case | Target | Expected Hard | Actual Hard | Policy | Aligned | Stale Ratio | CSD Ratio |",
        "|---|---|---:|---:|---|---:|---:|---:|",
    ]
    for row in report["cases"]:
        lines.append(
            "| {case_id} | {target} | {expected} | {hard} | `{policy}` | {aligned} | {stale_ratio} | {csd_ratio} |".format(
                case_id=row["case_id"],
    ...
    lines.append(
        "- Aggressive-case failures suggest the retrieval feature bridge is under-detecting stale/current conflict."
    )
    lines.append(
        "- Protect-case failures suggest the bridge or learned selector is over-escalating clean retrieval contexts."
    )
    lines.append(
        "- The next architecture improvement should tune the retrieval feature formulas only after reviewing the mismatched rows."
    )
    OUT_MD.write_text("\n".join(lines) + "\n", encoding="utf-8")
