...
DEFAULT_HERMES_EVAL = REPO_ROOT / "experiments" / "hermes_learned_risk_authority_boundary_logged_eval_results.json"
OUT_JSON = REPO_ROOT / "experiments" / "adaptive_residual_learned_risk_hermes_authority_boundary_replay_results.json"
OUT_MD = REPO_ROOT / "experiments" / "adaptive_residual_learned_risk_hermes_authority_boundary_replay_report.md"
...
def expected_label(row: dict[str, Any]) -> str:
    feedback = str(row.get("feedback_label") or "")
    query = str(row.get("query") or "").lower()
    if feedback == "answer_stale" or "prior no-veto" in query or "revert" in query:
...
def build_report(path: Path = DEFAULT_HERMES_EVAL) -> dict[str, Any]:
    ...
    for row in harmful:
        ...
        sample = make_sample(
            ...
            source="hermes_authority_boundary_failure_replay",
        )
    ...
    checks = {
        ...
        "source_eval_failed": source.get("ok") is False,
    ...
    return {
        "schema": "adaptive_residual_learned_risk_hermes_authority_boundary_replay/v1",
        "description": "Replay Hermes authority-boundary harmful residual decisions against the current learned-risk veto.",
...
def write_report(report: dict[str, Any]) -> None:
    ...
    OUT_MD.write_text(
        "# Adaptive Residual Learned Risk Hermes Authority Boundary Replay\n\n"
        + f"Passed: **{report['ok']}**\n"
